		...
		switch (prop) {
			...
			case 0x1D: // Alternate road type label list
				if (_cur.grffile->roadtype_map[basetype][id + i] != INVALID_ROADSUBTYPE) {
					int n = buf->ReadByte();
					for (int j = 0; j != n; j++) {
						_roadtypes[basetype][_cur.grffile->roadtype_map[basetype][id + i]].alternate_labels.push_back(BSWAP32(buf->ReadDWord()));
					}
					break;
				}
...
static void AfterLoadGRFs()
{
	...
	FOR_ALL_ENGINES_OF_TYPE(e, VEH_ROAD) {
		...
		if (file != NULL && _gted[e->index].roadsubtype < file->roadtype_list[rt].size()) {
			...
			e->u.road.roadsubtype = ROADSUBTYPE_NORMAL;
		}
