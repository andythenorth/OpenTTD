		...
		if (pt.x != -1) {
			switch (select_proc) {
				...
				case DDSP_PLACE_AUTOROAD:
					...
					DoCommandP(start_tile, end_tile, _place_road_flag | (_cur_roadtype_identifier.Pack() << 3) | (_one_way_button_clicked << 10),
...
DropDownList *GetRoadTypeDropDownList(RoadTypes roadtypes, bool for_replacement, bool all_option)
{
	...
	if (all_option) {
		list->push_back(new DropDownListStringItem(STR_REPLACE_ALL_ROADTYPE, -1, false));
	}
	...
	for (RoadType rt = ROADTYPE_BEGIN; rt < ROADTYPE_END; rt++) {
		...
		FOR_ALL_SORTED_ROADTYPES(rtid, rt) {
			...
			if (for_replacement) {
				item = new DropDownListParamStringItem(str, rtid.Pack(), !HasBit(c->avail_roadtypes[rtid.basetype], rtid.subtype));
			} else {
				DropDownListIconItem *iconitem = new DropDownListIconItem(rti->gui_sprites.build_x_road, PAL_NONE, str, rtid.Pack(), !HasBit(c->avail_roadtypes[rtid.basetype], rtid.subtype));
	...
	if (list->size() == 0) {
		...
		list->push_back(new DropDownListStringItem(STR_NONE, -1, true));
	}
