...
RoadTypeIdentifier AllocateRoadType(RoadTypeLabel label, RoadType basetype)
{
	...
	for (RoadSubType rt = ROADSUBTYPE_BEGIN; rt != ROADSUBTYPE_END; rt++) {
		...
		if (rti->label == 0) {
			...
			*rti = (basetype == ROADTYPE_ROAD ? _original_roadtype : _original_tramtype);
			rti->label = label;
 ...
 */
static bool CanConnectToRoad(TileIndex tile, RoadTypeIdentifier rtid, DiagDirection dir)
{
	tile += TileOffsByDiagDir(dir);
	if (!IsValidTile(tile) || !MayHaveRoad(tile)) return false;

	RoadTypeIdentifier existing = GetRoadType(tile, rtid.basetype);
	if (!existing.IsValid()) return false;
	if (!HasPowerOnRoad(existing, rtid) && !HasPowerOnRoad(rtid, existing)) return false;

	RoadBits bits = GetAnyRoadBits(tile, rtid.basetype, false);
...
CommandCost CmdBuildLongRoad(TileIndex start_tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	for (;;) {
		...
		if (!is_ai && start_tile != end_tile) {
			...
			if (tile == end_tile && !CanConnectToRoad(tile, rtid, dir)) {
				bits = DiagDirToRoadBits(ReverseDiagDir(dir));
			} else if (tile == start_tile && !CanConnectToRoad(tile, rtid, ReverseDiagDir(dir))) {
